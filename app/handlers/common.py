...
async def about_us(message: Message):
    ...
    await message.answer(
        """Вітаю! Ми(тобто я) студент фахового коледжу інформаційних технологій, першокурсник. Ця програма створена 1 людиною без використання ChatGPT для прямого написання\
 коду
(файл README.md не рахується, це чисто оформлення).
Робота несе з собою 2 мети.

Перша: Отримання мною досвіду в написанні телеграм ботів, роботи з парсингом, базами данних та іними технологіями в програмуванні.

Друга і основна: Допомога випускникам з низькими балами отримати шанс на бюджет без лишньої мороки."""
    )
    await message.answer(
        """А тепер трішки про сам проєкт і його історію:

Почалося все ще в 2024 році, коли ми з товаришем поступали (Я в коледж після 9, а він в університет після 11)
Мій період подачі заяв почався раніше і я зі спокійнею душею подав документи, здав вступні екзамени і... Написав невеликий скрипт для того, щоб продивлюватися які взагалі в мене шанси на поступання.
А от коли почався його період подачі заяв, він скинув мені ось це відео: https://www.youtube.com/watch?v=m5YfI8_2ONo
І написав, що вже 2 години фільтрує тільки 3 спеціальність і він розуміє, що це треба буде робити це саме ще завтра і може ще не один день.
...
Пізніше ми дописали туди tkinter для гарного виводу данних, але все ж, це залишалася локальна програма, яку потрібно запускати маючи пк.
Звісно, найбільший недолік цієї прогами був у складному запуску, інтерфейсі і тому, що потрібно спочатку скопіювати дані з сайту, але не зважаючи на це, вона повністю працює і зараз, можете завантажити з репозиторію і спробувати запустити.👇
...
Тут використовуються бази данних, парсери, телеграм API і ще купа всякого, а головне - вся програма є у відкритому доступі з відкритим вихідним кодом і росповсюджується під ліцензією GPLv3.
...
https://github.com/OlexiyOdarchuk/AbitAssistant_bot"""
    ...
    await message.answer(
        """Буду чекати там кожного і з радістю прийму всі ваші пропозиції для покращень!
